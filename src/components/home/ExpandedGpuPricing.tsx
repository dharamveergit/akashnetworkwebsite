...
/**
 * Optimized GPU Pricing Component
 * - Removed react-query and axios dependencies (saves ~50KB+ gzipped)
 * - Uses native fetch with simple caching
...
const gpuTypes = [
  ...
  },
  {
    ...
    mobileName: "Akash",
    selected: true,
    svg: `<svg width="100%" height="100%" viewBox="0 0 26 23" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M16.9224 15.141L20.961 22.2187H12.803L8.7218 15.141H16.9224Z" fill="url(#paint0_linear_84_299)"/>
      <path d="M20.9588 22.2195L25.0333 15.1417L16.8776 0.98291H8.7218L20.9588 22.2195Z" fill="currentColor"/>
      <path d="M4.64461 8.0592H12.8004L4.64801 22.2181L0.566772 15.1403L4.64461 8.0592Z" fill="currentColor"/>
      <defs><linearGradient id="paint0_linear_84_299" x1="18.5" y1="21.5" x2="21.5142" y2="18.5143" gradientUnits="userSpaceOnUse">
        <stop stop-color="currentColor"/><stop offset="1" stop-color="currentColor" stop-opacity="0"/>
      </linearGradient></defs>
    </svg>`,
  },
  {
  ...
  },
  {
    ...
    svg: `<svg width="100%" height="100%" viewBox="0 0 34 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <defs><clipPath id="clip0_177_345"><rect width="33.2" height="19.817" fill="currentColor" transform="translate(0.800049 0.192627)"/></clipPath></defs>
    </svg>`,
  },
];

// Static initial pricing - renders immediately without JS execution
...
const GPU_API_URL = "https://stats.akash.network/v1/gpu";
...
const ExpandedGpuPricing = () => {
  ...
  useEffect(() => {
    // Fetch prices after initial render (non-blocking)
    const fetchPrices = async () => {
      try {
        const response = await fetch(GPU_API_URL);
        if (!response.ok) return;

        const result: GpuData = await response.json();
        if (!result?.models) return;

        const newPrices = [...initialPricing];
        const h200Price = result.models.find((m) =>
          m.model?.toLowerCase().includes("h200"),
        )?.price?.avg;
        const h100Price = result.models.find((m) =>
          m.model?.toLowerCase().includes("h100"),
        )?.price?.avg;
        const a100Price = result.models.find((m) =>
          m.model?.toLowerCase().includes("a100"),
        )?.price?.avg;

        if (h200Price) newPrices[0][1] = `$${h200Price.toFixed(2)}`;
        if (h100Price) newPrices[1][1] = `$${h100Price.toFixed(2)}`;
        if (a100Price) newPrices[2][1] = `$${a100Price.toFixed(2)}`;

        setPrices(newPrices);
      } catch {
        // Silently fail - keep showing initial static prices
      }
    };

    // Use requestIdleCallback to fetch when browser is idle
    if ("requestIdleCallback" in window) {
      requestIdleCallback(() => fetchPrices());
    } else {
      // Fallback for Safari
        ...
        <div className="grid w-full grid-cols-5 rounded-t-lg border">
          ...
          {expanded && (
            <div className="col-span-5 flex flex-col">
              {prices.slice(1).map((item, rowIndex) => (
                <div className="grid grid-cols-5" key={rowIndex}>
                  {item.map((cell, i) => (
                    <h4
                      key={i}
                      className={clsx(
